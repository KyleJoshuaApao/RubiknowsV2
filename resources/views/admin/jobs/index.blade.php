    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div>
                ...
                <p class="text-sm font-bold uppercase tracking-widest text-gray-500 mt-2">Manage job postings and open positions.</p>
            </div>
            <div class="mt-4 sm:mt-0">
                <a href="{{ route('admin.jobs.create') }}" class="inline-flex items-center bg-brand-500 text-white px-4 py-2  shadow-sm hover:bg-brand-600 hover:shadow transition-all duration-200 font-medium text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="px-8 py-5 text-left text-xs font-black text-richblack-900 uppercase tracking-widest border-b border-gray-200">Position</th>
                        <th scope="col" class="px-8 py-5 text-left text-xs font-black text-richblack-900 uppercase tracking-widest border-b border-gray-200">Type / Location</th>
                        ...
                        <th scope="col" class="px-8 py-5 text-left text-xs font-black text-richblack-900 uppercase tracking-widest border-b border-gray-200">Applicants</th>
                ...
                <tbody class="bg-white divide-y divide-gray-50">
                    @forelse($jobs as $job)
                        <tr class="hover:bg-gray-50 transition-colors duration-150 group {{ $job->is_archived ? 'opacity-75' : '' }}">
                            <td class="px-8 py-6 whitespace-nowrap">
                                <div class="text-base font-black text-richblack-900 uppercase tracking-widest">{{ $job->title }}</div>
                                <div class="text-xs text-gray-500">Posted: {{ $job->created_at->format('M j, Y') }}</div>
                            </td>
                            <td class="px-8 py-6 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5  text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100 mb-1">
                                    {{ $job->type }}
                                </span>
                            ...
                            <td class="px-8 py-6 whitespace-nowrap">
                                @if($job->is_archived)
                                    <span class="inline-flex items-center px-2.5 py-1  text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">
                                        Archived (Closed)
                                    </span>
                                @else
                            ...
                            <td class="px-8 py-6 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                    <a href="{{ route('admin.jobs.edit', $job) }}" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50  transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form action="{{ route('admin.jobs.destroy', $job) }}" method="POST" class="inline-block" data-confirm="Are you sure you want to delete this job posting? This action cannot be undone.">
                                        @csrf
                                        @method('DELETE')
