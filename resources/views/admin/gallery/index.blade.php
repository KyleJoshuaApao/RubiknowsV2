    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div>
                ...
                <p class="text-sm font-bold uppercase tracking-widest text-gray-500 mt-2">Manage project photos, videos, and construction progress media.</p>
            </div>
            <div class="mt-4 sm:mt-0">
                <a href="{{ route('admin.gallery.create') }}" class="inline-flex items-center bg-brand-500 text-white px-4 py-2  shadow-sm hover:bg-brand-600 hover:shadow transition-all duration-200 font-medium text-sm">
                ...
                @foreach($media as $item)
                    <div class="bg-white  shadow-sm border border-gray-100 overflow-hidden group">
                        <div class="aspect-w-16 aspect-h-10 bg-gray-100 relative">
                            @if($item->type === 'Photo')
                                <img src="{{ \App\Support\MediaUrl::for($item->url) }}" alt="{{ $item->title }}" class="object-cover w-full h-48">
                            @else
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100 space-x-3">
                                <a href="{{ route('admin.gallery.edit', $item) }}" class="p-2 bg-white text-gray-900  hover:bg-brand-500 hover:text-white shadow-lg transition-colors">
                                ...
                                <form action="{{ route('admin.gallery.destroy', $item) }}" method="POST" class="inline-block" data-confirm="Delete this media item permanently? This action cannot be undone.">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 bg-white text-red-600  hover:bg-red-600 hover:text-white shadow-lg transition-colors">
